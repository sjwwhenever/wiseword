...
ALPHA_VANTAGE_API_KEY = "RHI2ZBXUM5JVWFU7"
#GNEWS_API_KEY = os.getenv('GNEWS_API_KEY')
GNEWS_API_KEY = "4b4567e53248b2607df3e4eb509222e8"
...
async def fetch_news(company_name): # get news urls and description for the given news company or ticker
    url = f"https://gnews.io/api/v4/search?q={company_name}&lang=en&country=us&max=10&apikey={GNEWS_API_KEY}"
    response = requests.get(url)
    logging.info(f"Fetching news from URL: {url}")
    logging.info(f"Response status code: {response.status_code}")
    articles = response.json().get('articles', [])
    logging.info(f"Articles received: {articles}")
    ...
    logging.info(f"News list created: {news_list}")
    return news_list
